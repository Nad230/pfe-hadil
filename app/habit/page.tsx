...
import { EnhancedDisciplineScore } from "@/components/habits copy/enhanced-discipline-score"
import { BadgesDisplay } from "@/components/habits copy/badges-display"
import { WeeklyHeatmap } from "@/components/habits copy/weekly-heatmap"
import { WeeklyReview } from "@/components/habits copy/weekly-review"
import { WeeklyAdvancedView } from "@/components/habits copy/weekly-advanced-view"
import { DailyCheckIn } from "@/components/habits copy/daily-check-in"
import { BadgeProgress } from "@/components/habits copy/badge-progress"
import { HabitJournal } from "@/components/habits copy/habit-journal"
    ...
    setHabits((prevHabits) =>
      prevHabits.map((habit) => {
        if (habit.id === habitId) {
          ...
          const todayCompletionIndex = existingCompletions.findIndex(
          ...
          if (todayCompletionIndex >= 0) {
            ...
            newCompletions[todayCompletionIndex] = {
              ...newCompletions[todayCompletionIndex],
              completed,
              notes: notes || newCompletions[todayCompletionIndex].notes,
            }
          } else {
            // Add new completion
            newCompletions.push({
          ...
          return {
            ...
            streak: completed ? habit.streak + 1 : 0,
          }
        }
        return habit
      }),
      ...
      .map((habit) => {
        ...
        const progress =
          habit.weeklyTarget > 0
            ? Math.min(
                100,
                Math.round((recentCompletions.filter((c) => c.completed).length / habit.weeklyTarget) * 100),
              )
            : 0
              ...
              <>
                ...
                <Tabs value={activeTab} className="mb-4" onValueChange={setActiveTab}>
                  ...
                  <TabsContent value="overview">
                    {/* Daily Check-in */}
                    <div className="mb-8">
                      <DailyCheckIn habits={habits} onCheckIn={handleHabitCheckIn} />
                    </div>
