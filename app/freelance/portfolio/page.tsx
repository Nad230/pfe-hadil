...
export default function PortfolioPage() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      setProjects(Array.isArray(data) ? data : []) // fallback to []
    } catch (error) {
      console.error("Failed to load projects", error)
      setProjects([]) // ← fallback to empty array so it won't show error
      // optionally: setError("Unable to load projects") if you want
    } finally {
      setLoading(false)
    }
  }
