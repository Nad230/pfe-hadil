...
function formatStat(value: number, type: 'time' | 'currency') {
  ...
  const millimes = value * 10000; // Convert decimal to millimes
  if (millimes >= 10000) {
    const dinars = millimes / 1000;
    return `${dinars.toFixed(0)}d`;
  }
  return `${millimes.toFixed(0)}ml`;
}
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetchTodayData()
        ...
        // Schedule a post request just before midnight
       
    
        const timeout = setTimeout(() => {
          const sendProfitData = async () => {
            try {
              const token = Cookies.get("token")
              if (!token) {
                console.warn("No token found for POST request")
                return
              }
              ...
              const payload = {
                ...
                day: days[new Date().getDay()] // Get current day name
              }
          
              const res = await fetch("http://localhost:3000/sales/profit", {
                method: "POST",
                headers: {
              ...
              if (!res.ok) {
                throw new Error("Failed to send profit data")
              }
          
              console.log("✅ Profit data sent successfully at end of day")
            } catch (err) {
              console.error("❌ Error sending profit data:", err)
            }
          }
